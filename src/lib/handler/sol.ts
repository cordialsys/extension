...
async function locateSignedTx(
  ...
  while (Date.now() - start < timeoutMs) {
    const result: Result<T.Transaction> = await Sdk.treasury.treasuryApiGet(txName);

    if (!result.ok) {
      return result; // propagate error
    }

    if (result.value.state === "submitting") {
...
export async function signIn(
  ...
  const callName = callNameResult.value.name!;

  // 3. Wait for the call to complete
  const callResult: Result<T.Call> = await Sdk.treasury.chains.calls.get(callName);
...
export async function signTransaction(
  ...
  // Uint8Array is not what it appears to be
  // Solana WalletStandard expects Uint8Array from lib.es5.d.ts
  // But since we use WXT, it gets imported from lib.es2015.iterable.d.ts
  // The type is almost the same, but it serializes to {0: 2, 1: 4, 2: 1} instead of [2, 4, 1]
  // TODO: Adjust tsconfig.json probably to fix that
  const txAsArray: number[] = Object.values(inputs[0].transaction) // Normalize the weird object structure
  const txAsBytes = new Uint8Array(txAsArray);
  const input = {
  ...
  if (!proposalNameResult.ok) {
    return proposalNameResult;
  }
  const proposalName = proposalNameResult.value;
  const callNameResult = await locateCall(proposalName);
  if (!callNameResult.ok) {
    return Err(Error.unknown(JSON.stringify(callNameResult.error)));
  }
  const callResource = callNameResult.value;
  const txResource = await locateSignedTx(callResource.transaction!);
  if (txResource.ok) {
    const payload = txResource.value.payload!;
    const bytes: number[] = payload
      .match(/.{1,2}/g)!
      .map(byte => parseInt(byte, 16));

    // The Uint8Array weirdness happens here too
    return Ok([{signedTransaction: bytes}]);
  } else {
    return Err(Error.unknown("failed to sign transaction"));
  }
}
