...
async function showProposal(proposalName: string, tab: number) {
  const context = SidePanel.proposalNavigation(
    proposalName,
    Config.treasuryId(),
  );
  await SidePanel.openNavigation(tab, context);
}
...
export async function eth_rpc(
  method: string,
  params: unknown,
): Promise<Result<unknown>> {
  if (!CONFIG) return Err(Error.permissionDenied("Not configured"));
  return await Sdk.connector.rpc(CONFIG.chain, method, params);
}
