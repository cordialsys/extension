(function(){
  ...
  function formatMaybeJson(x) {
    ...
    if (typeof x === 'string') {
      try { return JSON.stringify(JSON.parse(x), null, 2); } catch {}
      return x;
    }
  ...
  window.addEventListener('message', (event) => {
    ...
    if (data.type === 'connected') {
      connectStatus.textContent = 'Connected. window.ethereum is now available on this page.';
  ...
  sendBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      const res = await window.ethereum.request({ method: 'defi_sendTransfer', params: body });
